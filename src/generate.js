...
let trial_lines = function(vessel, band, signatures, n_noise) {
    ...
    for (var i = 0; i < n_lines; i++) {
        ...
        lines.push(((i * spacing) + Math.random() * spacing).toFixed(3));
    }
...
let signatures = function(n_lines) {
    let spacing = 1 / n_lines;
    ...
    for (var i = 0; i < n_lines; i++) {
        for (var sig = 0; sig < 2; sig++) {
            for (var band = 0; band < 2; band++) {
                vessel_signatures[sig][band].push(((i * spacing) + (Math.random() * spacing)).toFixed(3));
            }
        }
    }
    return vessel_signatures;
};

// Generate run order
let run_order = function(n_ff_trials, p_catch, n_noise, signatures, highlight) {
    ...
    for (var i = 0; i < run_order.length; i++) {
        ...
        var lines_trial = trial_lines(vessel, active_band, signatures, n_noise);
